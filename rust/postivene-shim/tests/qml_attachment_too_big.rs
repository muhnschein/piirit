...
//! (`POSTIVENE_FAKE_MSGSIZE_MAX`), so the files here are small enough to
...
fn a_file_the_relay_will_not_take_is_said_so_and_cannot_be_sent() {
    let temp = std::env::temp_dir().join(format!("postivene-qml-too-big-{}", std::process::id()));
    ...
    unsafe {
        ...
        std::env::set_var("POSTIVENE_FAKE_MSGSIZE_MAX", "1024");
    }
