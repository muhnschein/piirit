...
// See piiri-shim/src/lib.rs: qmetaobject's macros expand to references
...
/// `PIIRI_QML_DIR`, then the installed path, then the source tree.
fn qml_dir() -> PathBuf {
    if let Ok(env) = std::env::var("PIIRI_QML_DIR") {
        return PathBuf::from(env);
    }
    let installed = PathBuf::from("/usr/share/harbour-piiri/qml");
    if installed.is_dir() {
        return installed;
    }
...
/// Where the compiled catalogs are: `PIIRI_TRANSLATIONS_DIR`, then
...
fn translations_dir() -> PathBuf {
    if let Ok(env) = std::env::var("PIIRI_TRANSLATIONS_DIR") {
        return PathBuf::from(env);
    }
    let installed = PathBuf::from("/usr/share/harbour-piiri/translations");
    if installed.is_dir() {
        return installed;
    }
...
fn main() {
    // `PIIRI_MEMORY_LOG=<seconds>`: what the app and the core weigh,
    ...
    // `--rpc-server <path>`, then `PIIRI_RPC_SERVER`, then the bundled
    // binary -- and never `PATH`; see `piiri_shim::server_path`.
    ...
    // honoured.
    let server = piiri_shim::server_path(
        std::env::args().skip(1),
        std::env::var("PIIRI_RPC_SERVER").ok(),
    );
    view.engine()
        .set_property("rpcServerPath".into(), QString::from(server).into());

    let main_qml = qml_dir().join("piiri.qml");
