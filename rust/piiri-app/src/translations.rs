...
//! app's, from `translations/piiri-<lang>.qm`, which the RPM builds
...
//! the app under: `piiri-de_DE.qm` if there is one, else
//! `piiri-de.qm`, and nothing at all for a language the app has no
...
pub fn install(dir: &str, locale: &str) -> bool {
    ...
    cpp!(unsafe [dir as "QString", locale as "QString", name as "QString"] -> bool as "bool" {
        ...
        // form down, so piiri-pt_BR.qm wins over piiri-pt.qm where
        // both exist.
...
mod tests {
    ...
    fn compile(language: &str, out: &std::path::Path) -> Option<()> {
        ...
        let source = translations_dir().join(format!("piiri-{language}.ts"));
        let target = out.join(format!("piiri-{language}.qm"));
    ...
    fn the_readers_language_is_loaded_and_its_plurals_counted() {
        let out = std::env::temp_dir().join(format!("piiri-translations-{}", std::process::id()));
