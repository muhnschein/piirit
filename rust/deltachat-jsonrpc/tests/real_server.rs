...
async fn offline_round_trip_against_real_core() {
    ...
    let accounts_dir =
        std::env::temp_dir().join(format!("piiri-real-server-test-{}", std::process::id()));
    ...
    client
        .call::<_, ()>(
            "set_config",
            (account_id, "displayname", Some("Piiri Test")),
        )
        .await
        .expect("set_config");
    let name: Option<String> = client
        ...
        .await
        .expect("get_config");
    assert_eq!(name.as_deref(), Some("Piiri Test"));
    ...
    let attachment = std::env::temp_dir().join("piiri-real-server-note.txt");
    ...
    let picture = std::env::temp_dir().join("piiri-real-server-dot.png");
    ...
    // names a picture before sending it (piiri-shim/src/media.rs):
    ...
    let wide = std::env::temp_dir().join("piiri-real-server-wide.png");
    ...
    let animation = std::env::temp_dir().join("piiri-real-server-dot.gif");
    ...
    let tone = std::env::temp_dir().join("piiri-real-server-tone.wav");
    ...
    // built on (piiri-shim/src/chat_media.rs): up to three view types
    ...
    let card: String = client
        ...
        .await
        .expect("make_vcard");
    let card_path = std::env::temp_dir().join("piiri-real-server-ada.vcf");
    ...
    // `get_webxdc_blob` (piiri-shim/src/webxdc_host.rs), so a wrong
    // assumption about any of these is a blank page on a phone and
    // nothing anywhere else.
    let app_file = std::env::temp_dir().join("piiri-real-server-checkers.xdc");
    ...
    // the exclusion, and `piiri-shim/tests/blocking.rs` drives this
    // side of it.
    client
    ...
    let avatar = std::env::temp_dir().join("piiri-real-server-avatar.png");
