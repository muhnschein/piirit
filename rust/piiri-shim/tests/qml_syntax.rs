...
fn delegates_bind_only_roles_their_models_have() {
    ...
    let cases: [(&str, Vec<String>); 13] = [
        (
            ...
            names_of::<piiri_shim::MessageListItem>(),
        ),
        ...
        (
            ...
            names_of::<piiri_shim::ContactItem>(),
        ),
        (
        ...
        (
            ...
            names_of::<piiri_shim::SearchItem>(),
        ),
    ];
...
fn the_new_group_page_reads_only_what_a_contact_row_carries() {
    ...
    let names = names_of::<piiri_shim::ContactItem>();
...
fn list_pages_clip_and_leave_room_for_what_sits_below_them() {
    for (page, list) in [
        ...
        // "stopped" says "restart Piiri" through a reconnection that is
        // already under way.
...
fn only_the_settings_object_names_the_dconf_keys() {
    ...
    assert!(
        keys.iter()
            .all(|key| key.starts_with("/apps/harbour-piiri/")),
...
fn the_share_methods_and_what_answers_them_agree() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
    let desktop =
        fs::read_to_string(root.join("harbour-piiri.desktop")).expect("read the desktop entry");
    let target =
...
fn only_the_share_target_imports_sailfish_share() {
    ...
    assert!(
        ...
        "these import Sailfish.Share outside ShareTarget.qml, so a release \
         ...
         from a Loader instead, as piiri.qml does:\n  {}",
        offenders.join("\n  ")
    );
}
...
fn qml_reads_no_name_that_is_not_there() {
    ...
    const IN_SCOPE: [&str; 20] = [
        ...
        "core",
        "pageStack",
        // The root window's id in piiri.qml, in scope for every
