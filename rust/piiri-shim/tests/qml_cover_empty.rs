...
fn a_cover_with_only_oneself_and_the_device_says_there_are_no_messages() {
    let temp = std::env::temp_dir().join(format!("piiri-qml-cover-empty-{}", std::process::id()));
    ...
    unsafe {
        ...
        std::env::set_var("PIIRI_FAKE_ACCOUNTS", "1");
        std::env::set_var("PIIRI_FAKE_SELF_TALK", "1");
        std::env::set_var("PIIRI_FAKE_DEVICE_TALK", "2");
    }
