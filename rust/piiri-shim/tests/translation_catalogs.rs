...
//! from the diff.
//!
//! `piiri.ts` is the untranslated source catalog, and `piiri-en.ts`
...
fn every_language_catalog_is_complete() {
    ...
    let (wanted, _) =
        read_catalog(&fs::read_to_string(dir.join("piiri.ts")).expect("read the source catalog"));
    assert!(
        wanted.len() > 100,
    ...
    let mut catalogs: Vec<PathBuf> = fs::read_dir(&dir)
        ...
        .filter(|path| {
            ...
            name.is_some_and(|n| n.starts_with("piiri-"))
    ...
    for path in &catalogs {
        ...
        let language = name.trim_start_matches("piiri-").trim_end_matches(".ts");
