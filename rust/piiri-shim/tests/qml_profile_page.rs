...
fn the_profile_page_round_trips_the_profile() {
    let temp = std::env::temp_dir().join(format!("piiri-profile-page-{}", std::process::id()));
    ...
    single_shot(Duration::from_secs(10), move || unsafe {
        ...
        record!(
            "picked",
            call!("pick", QString::from("file:///tmp/piiri-fake/photo.jpg"))
        );
    });
