...
fn the_archived_list_is_not_overwritten_by_a_slow_ordinary_one() {
    let temp = std::env::temp_dir().join(format!("piiri-archived-race-{}", std::process::id()));
    ...
    unsafe {
        ...
        std::env::set_var("PIIRI_FAKE_JOURNAL", temp.join("journal.ndjson"));
        // Long enough to land well after the archived answer.
        std::env::set_var("PIIRI_FAKE_CHATLIST_DELAY_MS", "1500");
    }
