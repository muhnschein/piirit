...
//! and the app was Postivene before it was Piiri, so a profile from
...
fn a_profile_from_the_old_name_is_adopted_but_never_overwrites_one() {
    let root = std::env::temp_dir().join(format!("piiri-accounts-{}", std::process::id()));
    let _ = fs::remove_dir_all(&root);
    let data = root.join("share");

    // SAFETY: single-threaded test binary.
    unsafe {
        std::env::remove_var("PIIRI_ACCOUNTS_DIR");
    ...
    let wanted = data.join("piiri/piiri/accounts");
    assert_eq!(
        Path::new(&chosen),
        wanted,
