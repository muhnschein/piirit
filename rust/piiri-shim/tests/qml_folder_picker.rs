...
fn a_folder_is_chosen_by_walking_to_it_and_taken_only_on_accept() {
    let temp = std::env::temp_dir().join(format!("piiri-folder-picker-{}", std::process::id()));
    ...
    let piiri = documents.join("Piiri");
    let piiri_str = piiri.to_string_lossy().into_owned();
    let alpha = documents.join("Alpha").to_string_lossy().into_owned();
    let alpha_for_probe = alpha.clone();
    let piiri_for_probe = piiri_str.clone();
    ...
    single_shot(Duration::from_secs(1), move || unsafe {
        ...
        record!(
            ...
            call!(
                ...
                QString::from(piiri_for_probe.clone())
            )
        );
        ...
        // takes it.
        std::fs::create_dir_all(&piiri).expect("make the default folder");
        record!(
            "reload",
            call!(
                "load",
