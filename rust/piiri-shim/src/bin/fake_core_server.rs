...
//!
//! Every request is appended as one JSON line to `PIIRI_FAKE_JOURNAL`,
...
impl State {
    ...
    fn seed_chats(&mut self) {
        if self.chats.is_empty() {
            ...
            let long = std::env::var("PIIRI_FAKE_LONG_CHAT")
                .ok()
            ...
            if std::env::var_os("PIIRI_FAKE_MEDIA").is_some() {
            ...
            self.archived_order = if std::env::var("PIIRI_FAKE_NO_ARCHIVED").is_ok() {
                Vec::new()
            } else {
                vec![3]
            };
            ...
            if std::env::var_os("PIIRI_FAKE_REACTED").is_some() {
                self.reactions
                    .entry(1)
                    .or_default()
    ...
    /// `PIIRI_FAKE_SELF_SENT`: the rest of the suite leans on a sent
    ...
    fn full_message(&self, msg: u64) -> Value {
        ...
        if std::env::var_os("PIIRI_FAKE_SELF_SENT").is_some() && self.sent.contains(&id) {
            message["fromId"] = json!(SELF);
        }
        ...
        let held_back = std::env::var("PIIRI_FAKE_HELD_BACK_MSG")
            .ok()
...
fn journal(method: &str, params: &Value) {
    let Ok(path) = std::env::var("PIIRI_FAKE_JOURNAL") else {
        return;
    };
...
/// What a message says.
///
/// Short by default, because most tests assert on it. `PIIRI_FAKE_WORDY`
...
fn wordy(msg: u64) -> String {
    ...
    if std::env::var_os("PIIRI_FAKE_WORDY").is_none() {
        return text;
    }
...
fn message_object(msg: u64) -> Value {
    ...
    let info_first = msg == 1 && std::env::var_os("PIIRI_FAKE_INFO_FIRST").is_some();
    ...
    if msg == 10 {
        ...
        message["file"] = json!("/tmp/piiri-fake/photo.jpg");
    ...
    // that is a video: in the group under PIIRI_FAKE_MEDIA, and by
    ...
    match msg {
        12 => {
            ...
            message["file"] = json!("/tmp/piiri-fake/6f5902ac237024bdd0c176cb93063dc4.pdf");
        ...
        13 => {
            ...
            message["file"] = json!("/tmp/piiri-fake/voice.aac");
        ...
        14 => {
            ...
            message["file"] = json!("/tmp/piiri-fake/checkers.xdc");
        ...
        15 => {
            ...
            message["file"] = json!("/tmp/piiri-fake/clip.mp4");
...
/// is answered only after `PIIRI_FAKE_SLOW_MS` (three seconds by
...
async fn add_transport_from_qr(
    ...
    let stopped = if qr.contains("slow") {
        tokio::time::sleep(delay_or("PIIRI_FAKE_SLOW_MS", 3000)).await;
...
async fn serve() {
    ...
    if let Ok(kinds) = std::env::var("PIIRI_FAKE_SEED_EVENTS") {
        let mut state = state.lock().await;
    ...
    if let Ok(after) = std::env::var("PIIRI_FAKE_EXIT_AFTER_MS") {
    ...
    if let Ok(after) = std::env::var("PIIRI_FAKE_LATE_ARRIVAL_MS") {
        if let Ok(millis) = after.parse() {
        ...
        tokio::spawn(async move {
            ...
            let response = match method.as_str() {
                ...
                "get_all_accounts" => {
                    ...
                    // configured, named in PIIRI_FAKE_ACCOUNTS. The
                    // onboarding tests leave it unset and start from none.
                    if state.accounts.is_empty() {
                        for account in env_ids("PIIRI_FAKE_ACCOUNTS") {
                ...
                "get_chatlist_entries" => {
                    ...
                    if !archived_only {
                        if let Some(delay) = std::env::var("PIIRI_FAKE_CHATLIST_DELAY_MS")
                            .ok()
                            .and_then(|value| value.parse::<u64>().ok())
                        {
                            drop(state);
                    ...
                    for chat in ids {
                        ...
                        items.insert(
                            ...
                            json!({
                                ...
                                "isSelfTalk": env_ids("PIIRI_FAKE_SELF_TALK").contains(&chat),
                                "isDeviceTalk": env_ids("PIIRI_FAKE_DEVICE_TALK").contains(&chat),
                ...
                "get_message_list_items" => {
                    ...
                    tokio::time::sleep(delay("PIIRI_FAKE_FETCH_DELAY_MS")).await;
                    let mut state = state.lock().await;
                    state.seed_chats();
                    let chat = positional(1)
                        .as_u64()
                ...
                "misc_send_msg" => {
                    ...
                    if should_fail(&text) {
                        ...
                        // sent row.
                        tokio::time::sleep(delay("PIIRI_FAKE_SEND_DELAY_MS")).await;
                ...
                "get_next_event_batch" => {
                    ...
                    loop {
                        ...
                        if !queued.is_empty() {
                            ...
                            // fails on another.
                            tokio::time::sleep(delay("PIIRI_FAKE_EVENT_DELAY_MS")).await;
