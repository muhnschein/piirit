...
mod tests {
    ...
    fn only_a_file_in_the_outbox_itself_is_ever_deleted() {
        let dir = std::path::Path::new("/cache/piiri/webxdc/outbox/5");
        let inside = |name: &str| in_outbox(dir, std::path::Path::new(name));

        assert!(inside("/cache/piiri/webxdc/outbox/5/notes.txt"));
        // A name the app chose is kept whole, quirks and all.
        assert!(inside("/cache/piiri/webxdc/outbox/5/two words.mp4"));

        // Another app's outbox, which is another app's business.
        assert!(!inside("/cache/piiri/webxdc/outbox/6/notes.txt"));
        // The outbox itself, and the cache around it.
        assert!(!inside("/cache/piiri/webxdc/outbox/5"));
        assert!(!inside("/cache/piiri/webxdc/outbox"));
        // Below it rather than in it.
        assert!(!inside("/cache/piiri/webxdc/outbox/5/deeper/notes.txt"));
        // The reader's own files, however the path is written.
        assert!(!inside("/elsewhere/Downloads/notes.txt"));
        assert!(!inside(
            "/cache/piiri/webxdc/outbox/5/../../../../etc/passwd"
        ));
