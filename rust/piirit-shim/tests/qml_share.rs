...
fn what_is_shared_reaches_the_chat_the_reader_picks() {
    let temp = std::env::temp_dir().join(format!("piirit-qml-share-{}", std::process::id()));
    ...
    single_shot(Duration::from_secs(1), move || unsafe {
        ...
        record!(
            "file",
            call!("shareFile", QString::from("/tmp/piirit-shared.png"))
        );
    ...
    single_shot(Duration::from_secs(3), move || unsafe {
        ...
        record!(
            ...
            call!(
                "intoWindow",
                QString::from("/tmp/piirit-shared.png"),
                QString::from("")
            )
        );
        record!(
    ...
    assert_eq!(
        value("file-said"),
        "activate;share:/tmp/piirit-shared.png|;",
    ...
    assert!(
        stack.contains(
            "replace:ConversationPage.qml{accountId=7,chatId=3,chatName=Ada,\
             sharedFile=/tmp/piirit-shared.png,sharedText=}"
        ),
