...
fn the_top_of_the_view_is_the_first_message_in_the_chat() {
    let temp = std::env::temp_dir().join(format!("piirit-qml-paging-{}", std::process::id()));
    ...
    unsafe {
        ...
        std::env::set_var("PIIRIT_FAKE_INFO_FIRST", "1");
    }
