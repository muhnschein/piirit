# Sailfish/OBS packaging for Piiri.
#
...
# the architectural and build-tooling template -- but simplified: Piiri
...
# server supplied via PIIRI_RPC_SERVER instead. Every device arch
...
Version:    0.1.0
Release:    1
# Piiri's own code is GPL-3.0-or-later; the bundled
...
%endif
Group:      Qt/Qt
URL:        https://github.com/muhnschein/piiri
...
# QAudioRecorder, for the voice recorder (piiri-shim/src/recorder.rs).
...
%description
Piiri is a Silica/QML SailfishOS client for Delta Chat. It contributes
...
# The translation catalogs: translations/piiri-<lang>.ts, compiled
...
# piiri-app's qml_dir() lookup (PIIRI_QML_DIR env var, then this
...
# The compiled catalogs, beside the QML: piiri-app looks for them at
# this path, the way it looks for qml/ above. Only the .qm files; the .ts
# sources they were built from stay in the tree.
(cd translations && find . -type f -name 'piiri-*.qm' -exec \
...
# PATH -- piiri-app defaults to this exact path.
#
...
# LICENSE is Piiri's own GPLv3 text. SOURCE.md discharges MPL-2.0
