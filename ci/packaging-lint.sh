...
if command -v rpmspec >/dev/null 2>&1; then
    ...
    if rpmspec -P "$root/rpm/harbour-piiri.spec" >/dev/null; then
        echo "packaging-lint: rpm/harbour-piiri.spec parses"
    else
        echo "packaging-lint: FAIL rpm/harbour-piiri.spec does not parse" >&2
        status=1
    fi
else
...
if command -v desktop-file-validate >/dev/null 2>&1; then
    ...
    out=$(desktop-file-validate "$root/harbour-piiri.desktop" 2>&1 |
        grep -v 'value "silica-qt5" for key "X-Nemo-Application-Type"' |
        grep -v 'key "X-Nemo-Application-Type" .* is not known' || true)
    if [ -z "$out" ]; then
        echo "packaging-lint: harbour-piiri.desktop valid"
    else
        echo "$out" >&2
        echo "packaging-lint: FAIL harbour-piiri.desktop" >&2
        status=1
    fi
else
...
if command -v lrelease >/dev/null 2>&1 || command -v lrelease-qt5 >/dev/null 2>&1; then
    ...
    if out=$("$root/scripts/release-translations.sh" "$qm_dir" 2>&1) &&
        ...
        echo "packaging-lint: every translations/piiri-*.ts compiles cleanly"
    else
        echo "$out" >&2
