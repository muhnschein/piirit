...
cargo_bin=$(sed -n '/^\[\[bin\]\]/,/^\[/p' "$root/rust/piiri-app/Cargo.toml" |
    sed -n 's/^name = "\(.*\)"/\1/p' | head -1)
if [[ "$cargo_bin" = "$name" ]]; then
    note "[1.2.3] the cargo binary is named '$name'"
else
    fail 1.2.3 "$cargo_bin" \
        "piiri-app's [[bin]] name must be '$name', the name the spec installs to /usr/bin"
fi
...
if [[ -z "$binary" ]]; then
    skip 1.7.3 "no built binary (run: cd rust && cargo build -p piiri-app)"
